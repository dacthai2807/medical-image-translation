...
from datasets import get_dataset,rescale,inverse_rescale,get_PET_dataset,unscale_image
...
class EGSDE(object):
    ...
    def egsde(self,):
        ...
        dataset = get_PET_dataset(pet_dir, test_flist)
        ...
        for id, (y, name) in enumerate(data_loader):
            logging.info(f'batch:{id}/{len(dataset) / args.batch_size}')
            n = y.size(0)
            x = y
            y0 = y.to(self.device)
            ...
            for it in range(args.sample_step):
                ...
                pred_images = []
                for b in range(n):
                    # gt_images.append(x[b].unsqueeze(0).permute(0, 2, 3, 1).cpu())
                    pred_images.append(y[b].unsqueeze(0).permute(0, 2, 3, 1).cpu())
                    # tvu.save_image(
                ...
                os.makedirs(os.path.join(self.args.samplepath, str(id)), exist_ok=True)
                # shape_str = "x".join([str(x) for x in gt_images.shape])
                # out_path = os.path.join(self.args.samplepath, str(id), f"gt_{shape_str}.npz")
                # np.savez(out_path, gt_images)
                shape_str = "x".join([str(x) for x in pred_images.shape])
                out_path = os.path.join(self.args.samplepath, str(id), f"pred_{shape_str}.npz")
